/******************************************************************************
 *  Compilation:  javac shift.java
 *  Execution:    java shift
 *  Dependencies: none
 *
...
public final class BinaryFileOut {
   ...
   /**
    ...
    private static void writeBit(boolean bit) {
        ...
        // add bit to buffer
        buffer <<= 1;  // 0011 0001 => 0110 0010 shifts in a zero from the right
        if (bit)
	    buffer |= 1;
   ...
   /**
    ...
    private static void clearBuffer() {
        ...
        if (n == 0) return;
        if (n > 0)
	    buffer <<= (8 - n); // scoot left if fewer than 8 bits ready to go 
        try {
            out.write(buffer);
        }
        catch (IOException e) {
